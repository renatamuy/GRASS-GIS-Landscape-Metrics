...
# WARNING! GRASS GIS does not like col names longer than 10 characters, so be very concise!!
# Or later you can do something like 
# grass.run_command('v.db.renamecolumn', map=shape_name, column='oldcolname,newcolname')
#grass.run_command('v.db.renamecolumn', map=shape_name, column='p_euc_2002,p_eu_2001')
...
#------------------
# 4.2.
# Running for proportion of eucaliptus for only 4 years - 2001-2004
...
# Create new cols
cols = ['p_euc_2001', 'p_euc_2002', 'p_euc_2003', 'p_euc_2004'] # Col name
...
# Print total time
print 'The zonal stats for prop of habitat for 4 years took '+str((end - start)/60)+' minutes.'
...
# Print total time
print 'The zonal stats for number of patches for 4 years took '+str((end - start)/60)+' minutes.'

# Export shapefile
os.chdir(input_dir)
# Export shapefile. If you already run proportion of Eucalyptus, the shape_name will have both metrics calculated: zonal number of patches and zonal proportion  
grass.run_command('v.out.ogr', input = shape_name, output = shape_name+'_prop_euca_np.shp', overwrite = True)

#-------------------------------------------------------------------------
# Do not run below!!!!!!!!!!!!!!!!!!!!!! NOT RUN!
# Other tests
python 
